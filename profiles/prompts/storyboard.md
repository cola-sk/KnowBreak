...
- 每个 shot 包含：narration(=对应 line 的 text), visual(画面描述，给真人讲/信息图/医学或科学示意/实拍), broll(B-roll 素材建议), subtitle(生成阶段必须等于 narration，后续人工编辑时可单独修改或清空), duration(=line 的 estimated_seconds)
...
- 生成分镜时 subtitle 必须与 narration 保持一致，不要精简、改写或留空；屏幕字幕差异化只允许后续人工审核阶段编辑
