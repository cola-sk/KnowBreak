...
- 如果对应 narration 给出了明确历史地名及今天位置，shot 2 的 visual 必须保留这层空间信息，可用古代地域示意、城郭远景或克制的地图定位表达“历史地名 → 今地名”；不要用现代城市街景冒充古代案发现场。
- 如果 narration 没有可靠的具体地点，visual 只表现地点类型，不得自行添加县名、府名或今地名。
